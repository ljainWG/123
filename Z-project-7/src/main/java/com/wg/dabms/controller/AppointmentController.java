...
public class AppointmentController {
	...
	public void bookAppointment(User currentUser) {
		LOGGER.info("User " + currentUser.getUsername() + " is trying to book an appointment.");
		...
		if (currentUser.getRole() != Role.PATIENT && currentUser.getRole() != Role.RECEPTIONIST
			...
			LOGGER.warning("Unauthorized booking attempt by user: " + currentUser.getUsername());
			return;
		}
		...
		if (doctors.isEmpty()) {
            LOGGER.info("No doctors found in department: " + department);
		...
		if (currentUser.getRole() == Role.PATIENT) {
			...
			if (patients.isEmpty()) {
                ...
                LOGGER.warning("No patients found for username: " + username);
				return;
			}
		...
		if (isBooked) {
			...
			LOGGER.warning("Failed to book appointment for patient ID: " + patientUuid);
		}
	}
	...
	public void updateAppointmentStatus(User currentUser) {        
		LOGGER.info("User " + currentUser.getUsername() + " is updating appointment status.");
		...
		if (currentUser.getRole() == Role.PATIENT) {
			...
			if (searchChoice == 1) {
				...
				if (patients.isEmpty()) {
					System.out.println("No patients found with that username.");
                    LOGGER.warning("No patients found for username: " + patientUsername);
					return;
				}
				printUserTable(patients);
				...
				if (doctors.isEmpty()) {
                    ...
                    LOGGER.warning("No doctors found for username: " + doctorUsername);
					return;
				}
		...
		if (appointments.isEmpty()) {
            ...
            LOGGER.info("No appointments found for user: " + currentUser.getUsername());
			return;
		}
