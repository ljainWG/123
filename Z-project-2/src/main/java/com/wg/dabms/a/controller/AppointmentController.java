...
public class AppointmentController {
	...
	private void printDoctorTable(List<User> doctors) {
		...
		for (int i = 0; i < doctors.size(); i++) {
			...
			// Handle null fields for the User object
	...
	public void updateAppointmentStatus(User currentUser) {
		...
		// If user is a patient, only show appointments where they are the patient.
		if (currentUser.getRole() == Role.PATIENT) {
			appointments = appointmentService.getAppointmentsByPatientId(currentUser.getUuid());
		}
		// If user is a doctor, only show appointments where they are the doctor.
		...
		// Logic to enforce status update rules based on the user role
		if (currentUser.getRole() == Role.PATIENT || currentUser.getRole() == Role.DOCTOR) {
			// Patients and doctors can only cancel appointments
			System.out.println("You can only cancel the appointment.");
			...
			// Receptionist can only update from SCHEDULED to other statuses
			...
			// Admin can update status to any valid status
			System.out.println("Select the new status:");
			System.out.println("1. COMPLETED");
			System.out.println("2. CANCELED");
			System.out.println("3. NO_SHOW_PATIENT");
	...
	public void viewAppointments(User currentUser) {
		...
		if (currentUser.getRole() == Role.PATIENT || currentUser.getRole() == Role.DOCTOR) {
			// For Patient and Doctor: Show only appointments where the user is either
			// patient or doctor
