...
public class PrescriptionController {
	...
	private void printPrescriptionList(List<Prescription> prescriptions) {
		System.out.printf("%-5s %-20s %-20s %-15s %-10s %-20s %-20s%n", "Index", "Doctor", "Patient",
				"Appointment Date", "Slot", "Creation Date", "Updation Date");
		...
		for (int i = 0; i < prescriptions.size(); i++) {
			...
			// Handle null checks for appointment
			if (appointment == null) {
				System.out.printf("%-5d %-20s %-20s %-15s %-10s %-20s %-20s%n", i + 1, "Unknown", "Unknown", "N/A",
						"N/A", prescription.getCreatedAt(), prescription.getUpdatedAt());
				continue;
			}

			// Retrieve doctor and patient names with null checks
			...
			System.out.printf("%-5d %-20s %-20s %-15s %-10s %-20s %-20s%n", i + 1, doctorName, patientName,
	...
	private void printAppointmentTable(List<Appointment> appointments) {
		...
		for (int i = 0; i < appointments.size(); i++) {
			...
			// Fetch doctor and patient details with null checks
			User doctor = userService.getUserById(appointment.getDoctorId());
			String doctorName = (doctor != null) ? doctor.getUsername() : "Unknown";

			User patient = userService.getUserById(appointment.getPatientId());
			String patientName = (patient != null) ? patient.getUsername() : "Unknown";
			...
			System.out.printf("%-5d %-20s %-20s %-12s %-10s %-20s %-20s%n", i + 1, doctorName, patientName,
					...
					appointment.getStatusUpdationDate() != null ? appointment.getStatusUpdationDate() : "N/A");
		}
	}
