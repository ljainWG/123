...
public class AppointmentServiceTest {
    ...
    public void testIsSlotAvailable() {
        ...
        TimeSlot slot = TimeSlot.SLOT_1030AM_11AM;
        when(appointmentDAO.isSlotAvailable(doctorId, date, slot)).thenReturn(true);

        boolean result = appointmentService.isSlotAvailable(doctorId, date, slot);
        assertTrue(result, "Should return true if slot is available");
        verify(appointmentDAO).isSlotAvailable(doctorId, date, slot);
    }
